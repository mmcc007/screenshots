...
Select the devices you want to run on, using a convenient config file. _Screenshots_ will find the device (real or emulated) and run your tests.
...
In addition to installed emulators/simulators, use your real devices to capture screenshots. Works even if the devices are unknown to _Screenshots_ if framing for each unknown device is disabled in config file.
...
||||This can also be used to record screenshots on any of your real devices or on any emulators/simulators that don't have supported screens in _Screenshots_.   |
